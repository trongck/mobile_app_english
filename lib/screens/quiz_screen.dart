      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          gradient: LinearGradient(
            colors: [grad[0].withOpacity(0.12), grad[1].withOpacity(0.06)],
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        if (widget.baiKT.tgLamPhut != null)
                          _Badge('${widget.baiKT.tgLamPhut} phút', grad[1]),
                      ],
                    ),
                  ],
                ),
    ...
    return GestureDetector(
      ...
      onTapUp: (_) {
        ...
        Navigator.push(
          ...
          MaterialPageRoute(
            builder: (_) =>
                XemLaiScreen(ls: widget.ls, provider: widget.provider),
          ),
        );
      },
      onTapCancel: () => setState(() => _pressed = false),
        ...
        child: Row(
          children: [
            Container(
              ...
              height: 44,
              decoration: BoxDecoration(
                color: _diemColor.withOpacity(0.12),
                borderRadius: BorderRadius.circular(12),
              ),
              child: Center(
                child: Text(
                  widget.ls.diem != null ? '${widget.ls.diem}' : '?',
                  style: TextStyle(
                    color: _diemColor,
                    fontSize: 16,
                    fontWeight: FontWeight.w900,
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  Text(
                    baiKT?.tieuDe ?? 'Bài kiểm tra #${widget.ls.maBKT}',
                    style: const TextStyle(
                      color: Colors.white,
                      fontSize: 14,
                      fontWeight: FontWeight.w700,
...
class _LamBaiScreenState extends State<LamBaiScreen>
  ...
  Future<void> _submit({bool autoSubmit = false}) async {
    ...
    for (final ch in _cauHoi) {
      ...
      if (answer != null &&
          answer.trim().toLowerCase() == ch.dapAn.trim().toLowerCase()) {
        tongDiem += ch.trongSo;
      }
    }
    final pct = maxDiem > 0
    ...
    if (mounted) {
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(
          builder: (_) => KetQuaScreen(
            ls: ls,
            cauHoi: _cauHoi,
            tongDiem: widget.baiKT.tongDiem,
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          '$answered/${_cauHoi.length} câu đã trả lời',
                          style: TextStyle(
                            color: Colors.white.withOpacity(0.45),
                            fontSize: 11,
                          ),
        ...
        return Column(
          children: [
            _DungSaiBtn(
              label: 'Đúng',
              value: 'Đúng',
              selected: _dapAn[ch.maCH],
              color: const Color(0xFF00FF94),
              onTap: () => setState(() => _dapAn[ch.maCH!] = 'Đúng'),
            ),
            ...
            _DungSaiBtn(
              ...
              onTap: () => setState(() => _dapAn[ch.maCH!] = 'Sai'),
            ),
          ],
        );
                      ...
                      Row(
                        children: [
                          Expanded(
                            child: _StatCard(
                              'Thời gian',
                              '${((widget.ls.tgLam ?? 0) / 60).toStringAsFixed(1)} phút',
                          ...
                          Expanded(
                            child: _StatCard(
                              'Đã trả lời',
                              '${widget.ls.cauTraLoi.length}/${widget.cauHoi.length}',
...
class _CauHoiKetQua extends StatelessWidget {
  ...
  const _CauHoiKetQua({
    required this.idx,
    required this.ch,
    required this.dapAnUser,
  });

  bool get _correct =>
      dapAnUser?.trim().toLowerCase() == ch.dapAn.trim().toLowerCase();
      ...
      child: Column(
        ...
        children: [
          ...
          if (!_correct) ...[
            const SizedBox(height: 8),
            Text(
              'Bạn chọn: ${dapAnUser ?? 'Chưa trả lời'}',
              style: TextStyle(
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: ListView.builder(
        ...
        itemBuilder: (_, i) => _CauHoiKetQua(
          idx: i,
          ch: cauHoi[i],
          dapAnUser: ls.cauTraLoi[cauHoi[i].maCH.toString()],
        ),
      ),
    );
  }
