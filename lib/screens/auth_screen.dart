...
class _AuthScreenState extends State<AuthScreen> with TickerProviderStateMixin {
        ...
        content: Row(
          children: [
            Icon(
              ...
              color: isError ? Colors.redAccent : const Color(0xFF00FF94),
              size: 18,
            ),
  ...
  Future<void> _loginGoogle() async {
    ...
    if (!mounted) return;
    if (ok) {
      _showToast('Xin chào ${p.nguoiDung?.hoTen ?? ''}!');
      await Future.delayed(const Duration(milliseconds: 500));
      _navigateToMain();
    } else {
      _showToast(p.error ?? 'Đăng nhập Google thất bại', isError: true);
      p.clearError();
    }
  }
  ...
  Future<void> _loginFacebook() async {
    ...
    if (ok) {
      ...
      _showToast(p.error ?? 'Đăng nhập Facebook thất bại', isError: true);
      p.clearError();
    }
  }
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        _isLogin
                            ? 'Tiếp tục hành trình học tiếng Anh IT'
                            : 'Chinh phục tiếng Anh IT ngay hôm nay',
                        style: TextStyle(
                        ...
                        Center(
                          child: GestureDetector(
                            ...
                            child: Container(
                              ...
                              child: Text(
                                _isLogin ? 'Tạo tài khoản mới' : 'Đăng nhập',
                                style: const TextStyle(
                                  color: Color(0xFF00D4FF),
          ...
          actions: [
            ...
            ElevatedButton(
              ...
              onPressed: isLoading
                  ? null
                  : () async {
                      final success = await widget.provider.xacMinhOtp(
                        _otpCtrl.text.trim(),
                      );
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: [
            BoxShadow(
              color: const Color(0xFF00D4FF).withOpacity(_p ? 0.2 : 0.32),
              blurRadius: 20,
...
class _BgPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final o in [
      [0.85, 0.12, 0.45, const Color(0xFF7B2FFF), 0.8],
      [0.15, 0.65, 0.4, const Color(0xFF00D4FF), 1.1],
      [0.6, 0.9, 0.3, const Color(0xFF0047FF), 0.6],
    ]) {
      final x =
          (o[0] as double) + math.sin(t * math.pi * 2 * (o[4] as double)) * 0.1;
      final y =
          (o[1] as double) +
          math.cos(t * math.pi * 2 * (o[4] as double)) * 0.08;
      p.shader =
          RadialGradient(
            colors: [(o[3] as Color).withOpacity(0.18), Colors.transparent],
          ).createShader(
            Rect.fromCircle(
